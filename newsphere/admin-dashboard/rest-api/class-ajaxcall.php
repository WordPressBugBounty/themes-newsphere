...
add_action('wp_ajax_newsphere_plugin_installer', 'newsphere_plugin_installer'); // Install plugin
add_action('wp_ajax_newsphere_plugin_activation', 'newsphere_plugin_activation'); // Activate plugin
add_action('wp_ajax_newsphere_plugin_installer_activation', 'newsphere_plugin_installer_activation'); // Activate plugin
function newsphere_plugin_installer()
{

  if (! current_user_can('install_plugins'))
    wp_die(__('Sorry, you are not allowed to install plugins on this site.', 'newsphere'));
  ...
  if (! wp_verify_nonce($nonce, 'aft_installer_nonce'))
    wp_die(__('Error - unable to verify nonce, please try again.', 'newsphere'));
...
function newsphere_plugin_activation()
{
  if (! current_user_can('install_plugins'))
    wp_die(__('Sorry, you are not allowed to activate plugins on this site.', 'newsphere'));
  ...
  // Check our nonce, if they don't match then bounce!
  if (! wp_verify_nonce($nonce, 'aft_installer_nonce'))
    die(__('Error - unable to verify nonce, please try again.', 'newsphere'));
  ...
  if ($api->name) {
    $main_plugin_file = newsphere_get_plugin_file($plugin);
    $status = 'success';
    if ($main_plugin_file) {
...
function newsphere_plugin_installer_activation()
{
  ...
  if (!empty($list_of_plugins)) {
    $data =  newsphere_activate_plugin_list($sanitinzed_activate);
  }
...
function newsphere_activate_plugin_list($sanitinzed_activate)
{
  $count = 0;
